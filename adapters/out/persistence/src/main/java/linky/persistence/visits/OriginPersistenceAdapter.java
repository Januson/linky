...
class OriginPersistenceAdapter implements PendingOrigins {
	...
	OriginPersistenceAdapter(final VisitsRepository visits, final VisitMapper mapper) {
		this.visits = visits;
		this.mapper = mapper;
	}
	...
	public List<Origin.Pending> all() {
		return this.visits.findAllByGeoEncoding(Origin.GeoEncoding.PENDING).map(this::toOrigin)
	...
	public void update(final List<Origin.Encoded> origins) {
		for (Origin origin : origins) {
			this.visits.updateOrigin(origin.geoEncoding(), origin.country().toString(),
					origin.ip().toString());
		}
		this.visits.flush();
	}

...
public class FindAllVisitsEndpoint {
	...
	@GetMapping(value = "/visits/{linkName}", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<List<VisitDto>> allOf(@PathVariable final String linkName,
			@RequestParam(name = "page", required = false, defaultValue = "0") int page,
			@RequestParam(name = "size", required = false, defaultValue = "10") int size) {
		final var pagination = new Pagination(page, size);
		return ResponseEntity.ok(createResponse(this.useCase.allOf(new Name(linkName))));
	}
